...
The `stack_type` config key controls how multiple toasts are displayed. Two modes are available: `:stack` (default) and `:collapse`.

### `:stack` (default)

Toasts appear as a vertical list. No extra CSS is required beyond your normal group layout — the existing flex column arrangement handles everything.

### `:collapse`

Toasts collapse behind the newest one. Hovering the group smoothly expands all toasts into a full stack, then collapses them again on mouse-out.

The group automatically measures its full content height on hover and sets `--expanded-height` for the CSS transition — no JavaScript needed in your own code.

Enable it via `Config.new!`:
...
Then add the collapse CSS. The group exposes two CSS custom properties:

- `--count` on the group container — total number of toasts currently shown
- `--idx` on each toast — position from the top (0 = newest)
- `--expanded-height` — set automatically on hover to the full content height

```css
[data-stack="collapse"] {
  overflow: hidden;
  /* Clipped height when collapsed: adjust to match your toast height */
  max-height: var(--expanded-height, 80px);
  transition: max-height 350ms cubic-bezier(0.4, 0, 0.2, 1);
}

[data-stack="collapse"] .hog-toast {
  transition: transform 350ms ease, opacity 350ms ease;
}

/* Toasts behind the newest are scaled down slightly to hint at depth */
[data-stack="collapse"] .hog-toast:not([data-idx="0"]) {
  transform: scale(calc(1 - var(--idx) * 0.03));
  transform-origin: bottom center;
  opacity: calc(1 - var(--idx) * 0.15);
}
```
...
The `80px` default for `max-height` is a reasonable starting point for a single toast. Adjust it to match your actual toast height — or set `--peek-height` on the group and reference it instead:

```css
.my-toast-group { --peek-height: 72px; }

[data-stack="collapse"] {
  max-height: var(--expanded-height, var(--peek-height, 80px));
}
```
